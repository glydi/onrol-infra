    ...
    final ok = await showFormSheet(context, title: r['name']?.toString() ?? 'Referral', builder: (setS) => [
      ...
      Wrap(spacing: 6, runSpacing: 6, children: [
        for (var i = 0; i < 5; i++) GestureDetector(
          onTap: () => setS(() => status = i),
          child: Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), decoration: BoxDecoration(color: status == i ? Palette.of(context).accent : Palette.of(context).card2, borderRadius: BorderRadius.circular(kRadiusField)),
