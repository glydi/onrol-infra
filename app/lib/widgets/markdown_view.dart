...
class MarkdownView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final base = GoogleFonts.ibmPlexMono(fontSize: baseFontSize, color: textColor, height: 1.65);
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (t.startsWith('### ')) {
        blocks.add(_pad(_rich(t.substring(4), GoogleFonts.ibmPlexMono(fontSize: 17, fontWeight: FontWeight.w700, color: textColor, height: 1.3)), top: 8));
        continue;
      }
      if (t.startsWith('## ')) {
        blocks.add(_pad(_rich(t.substring(3), GoogleFonts.ibmPlexMono(fontSize: 21, fontWeight: FontWeight.w800, color: textColor, height: 1.3)), top: 10));
        continue;
      }
      if (t.startsWith('# ')) {
        blocks.add(_pad(_rich(t.substring(2), GoogleFonts.ibmPlexMono(fontSize: 26, fontWeight: FontWeight.w800, color: textColor, height: 1.25)), top: 12));
        continue;
      }
