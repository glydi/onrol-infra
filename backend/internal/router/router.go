...
func Setup(app *fiber.App, h *handlers.Handlers, jwtm *auth.Manager, pool *pgxpool.Pool) {
	...
	api.Delete("/manage/videos", auth, mgr, h.DeleteVideo)
	...
	api.Get("/me/2fa", auth, h.TwoFAStatus)
	api.Post("/me/2fa/setup", auth, h.TwoFASetup)
	api.Post("/me/2fa/verify", auth, h.TwoFAVerify)
	api.Post("/me/2fa/disable", auth, h.TwoFADisable)
