...
func Setup(app *fiber.App, h *handlers.Handlers, jwtm *auth.Manager, pool *pgxpool.Pool) {
	...
	api.Patch("/manage/images/:id", auth, mgr, h.RenameImage)
	api.Post("/manage/images/:id/folder", auth, mgr, h.MoveImage)
	api.Delete("/manage/images/:id", auth, mgr, h.DeleteImage)
	api.Get("/manage/image-folders", auth, mgr, h.ListImageFolders)
	api.Post("/manage/image-folders", auth, mgr, h.CreateImageFolder)
	api.Patch("/manage/image-folders/:id", auth, mgr, h.RenameImageFolder)
	api.Delete("/manage/image-folders/:id", auth, mgr, h.DeleteImageFolder)
	...
	// Reusable module store (build once, add to any course/batch by code).
	api.Get("/manage/module-store", auth, inst, h.ListStoreModules)
	api.Post("/manage/module-store", auth, inst, h.CreateStoreModule)
	api.Get("/manage/module-store/:id", auth, inst, h.GetStoreModule)
	api.Delete("/manage/module-store/:id", auth, inst, h.DeleteStoreModule)
	...
	api.Post("/manage/modules/:id/move", auth, inst, h.MoveModule)  // manual reorder
	api.Post("/manage/lessons/:id/move", auth, inst, h.MoveLesson)  // manual reorder
	...
	api.Get("/manage/activity", auth, inst, h.StudentActivity)             // daily student interaction counts
	api.Post("/manage/mentor-questions/:id/ignore", auth, inst, h.IgnoreMentorQuestion)
	...
	// Live-host portal: lists every live class to host (answer Q&A + control).
	// The dedicated live_host role uses it, and admins/instructors get the same
	// all-courses list so any staff can enter any live host.
	api.Get("/live-host/sessions", auth, middleware.RequireAnyRole("live_host", "manager", "superadmin", "instructor"), h.ListLiveHostSessions)
