...
func (h *Handlers) ListCourses(c *fiber.Ctx) error {
	...
	q := `SELECT id, title, status, enroll_type, COALESCE(group_id::text,''), COALESCE(image_url,''), COALESCE(label,''), created_at, in_explore, COALESCE(explore_link,'') FROM courses`
	args := []any{}
	...
	out := []fiber.Map{}
	for r.Next() {
		var id, title, status, et, grp, img, label, link string
		var created any
		var inExplore bool
		if err := r.Scan(&id, &title, &status, &et, &grp, &img, &label, &created, &inExplore, &link); err != nil {
			return fiber.NewError(fiber.StatusInternalServerError, "scan failed")
		}
		out = append(out, fiber.Map{"id": id, "title": title, "status": status,
			"enroll_type": et, "group_id": grp, "image_url": img, "label": label, "created_at": created, "in_explore": inExplore, "explore_link": link})
	}
...
func (h *Handlers) AddModule(c *fiber.Ctx) error {
	...
	var req struct {
		...
		Code        string `json:"code"`             // module code (links to the store for sync)
	}
	if err := c.BodyParser(&req); err != nil || strings.TrimSpace(req.Title) == "" {
		return fiber.NewError(fiber.StatusBadRequest, "title required")
	}
	...
	var code any
	if cc := strings.ToUpper(strings.TrimSpace(req.Code)); cc != "" {
		code = cc
	}
	...
	var id string
	if err := h.Pool.QueryRow(c.Context(),
		`INSERT INTO modules (course_id, title, position, parent_module_id, batch_number, store_code) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id`,
		courseID, req.Title, req.Position, parent, batch, code).Scan(&id); err != nil {
		return fiber.NewError(fiber.StatusInternalServerError, "create failed")
	}
