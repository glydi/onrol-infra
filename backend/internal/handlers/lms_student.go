...
func (h *Handlers) MyCalendar(c *fiber.Ctx) error {
	...
	rows, err := h.Pool.Query(c.Context(), `
		SELECT 'session' AS kind, cs.title, cs.starts_at AS at, c.title AS course,
		       ...
		       COALESCE(cs.youtube_id,'') AS youtube_id,
		...
		UNION ALL
		SELECT 'assessment_due', a.title, a.due_at, c.title, ''::text, ''::text, ''::text, ''::text, ''::text, false
		...
		UNION ALL
		SELECT 'announcement', an.title, an.created_at, COALESCE(c.title,''), ''::text, ''::text, ''::text, ''::text, ''::text, false
		...
		UNION ALL
		SELECT 'event', e.title, e.starts_at, COALESCE(e.location,''), ''::text, ''::text, ''::text, ''::text, ''::text, false
	...
	out := []fiber.Map{}
	for rows.Next() {
		var kind, title, course, refID, liveKind, joinURL, webinarID, youtubeID string
		var at any
		var ended bool
		if err := rows.Scan(&kind, &title, &at, &course, &refID, &liveKind, &joinURL, &webinarID, &youtubeID, &ended); err != nil {
			return fiber.NewError(fiber.StatusInternalServerError, "scan failed")
		}
