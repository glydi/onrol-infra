...
func (h *Handlers) CreateSession(c *fiber.Ctx) error {
	...
	var req struct {
		...
		LiveHLSURL string `json:"live_hls_url"`
	...
	var startImg, endImg any
	if strings.TrimSpace(req.StartImage) != "" {
		startImg = req.StartImage
	}
	...
	var liveHLS any
	if u := strings.TrimSpace(req.LiveHLSURL); u != "" {
		liveHLS = u
	}
	var id string
	if err := h.Pool.QueryRow(c.Context(),
...
func (h *Handlers) UpdateSession(c *fiber.Ctx) error {
	...
	if _, err := h.Pool.Exec(c.Context(), `
		...
		SET title          = COALESCE($2, title),
		    ...
		    start_image    = COALESCE($10, start_image),
...
func (h *Handlers) ListCourseSessions(c *fiber.Ctx) error {
	...
	for rows.Next() {
		...
		var viewerBase, waiting int
		var startsAt any
		if err := rows.Scan(&id, &title, &startsAt, &joinURL, &hostURL, &loc, &mediaID, &chatOK, &qaOK, &viewerBase, &mediaTitle, &startImg, &endImg, &batch, &webinarID, &waiting); err != nil {
			return fiber.NewError(fiber.StatusInternalServerError, "scan failed")
		}
		kind := "external"
		if mediaID != "" {
		...
		out = append(out, fiber.Map{"id": id, "title": title, "starts_at": startsAt, "join_url": joinURL, "host_url": hostURL, "location": loc,
			...
			"start_image": startImg, "end_image": endImg, "batch": batch, "webinar_id": webinarID, "waiting": waiting})
	}
...
func (h *Handlers) MyLive(c *fiber.Ctx) error {
	rows, err := h.Pool.Query(c.Context(), `
		SELECT cs.id, cs.title, cs.starts_at,
		       ...
		       COALESCE(cs.webinar_id::text,''), COALESCE(cs.youtube_id,''), COALESCE(cs.live_hls_url,'')
		FROM class_sessions cs
	...
	out := []fiber.Map{}
	for rows.Next() {
		var id, title, joinURL, location, course, webinarID, youtubeID, liveHLS string
		var startsAt, endsAt any
		var simulated bool
		if err := rows.Scan(&id, &title, &startsAt, &endsAt, &joinURL, &location, &course, &simulated, &webinarID, &youtubeID, &liveHLS); err != nil {
			return fiber.NewError(fiber.StatusInternalServerError, "scan failed")
		}
		...
		if simulated {
			...
			kind = "livestream" // real live HLS feed, played in our live-room player
		} else if youtubeID != "" {
